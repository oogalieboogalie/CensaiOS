...
export async function ensureSubAgentTenancySchema(db = pool) {
  await db.query(`
...
export async function inspectSubAgentTenancySchema(db = pool) {
  const { rows } = await db.query(`
    SELECT table_name, count(*)::int AS scoped_columns
      FROM information_schema.columns
     WHERE table_schema = 'public'
       AND table_name IN ('sub_agents', 'sub_agent_scratchpad')
  ...
  return {
    ready: columns.sub_agents === 2 && columns.sub_agent_scratchpad === 2,
    columns,
  };
}
